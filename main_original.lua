...
local objectSheet = graphics.newImageSheet( "photos/gameObjects.png", sheetOptions )
...
-- Load the background
local background = display.newImageRect( backGroup, "photos/background.png", 800, 1400 )
	...
	if ( event.phase == "began" ) then
		...
		then
			if ( died == false ) then
				...
				if ( lives == 0 ) then
                    display.remove( ship )
                    local text = display.newText(mainGroup, "Ты проебал! ", display.contentCenterX,
                display.contentCenterY,font, 50 );
				else
					ship.alpha = 0
